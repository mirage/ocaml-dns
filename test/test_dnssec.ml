      ...
      begin
        ...
        | Some rrs ->
          ...
          | Some (_, dnskeys), Some (_, rrsigs) ->
            ...
            begin
              match Dnssec.dnskey_to_rsa used_dnskey.Dnskey.key with
              | Error _ -> Alcotest.fail "bad dnssec key"
              | Ok k ->
            ...
            begin
              match Dnssec.verify ts_of_req key Domain_name.root rrsig no_rrsig_rrmap with
              | Ok () -> ()
              | Error (`Msg m) ->
