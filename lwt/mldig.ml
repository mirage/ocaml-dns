...
let string_of_rr rr = sprintf "%-24s %-8lu %-8s %-8s %s\n"
  (string_of_name rr.name) rr.ttl (rr_class_to_string rr.cls)
...
let string_of_rrecord rr = 
  ...
  |MX (pref,host) ->
    string_of_rr rr "MX" (sprintf "%d %s" pref (string_of_name host));
  |CNAME a -> string_of_rr rr "CNAME" (string_of_name a)
  |NS a -> string_of_rr rr "NS" (string_of_name a)
...
let string_of_question q = 
  sprintf ";%-23s %-8s %-8s %s\n"
    (string_of_name q.q_name) "" (q_class_to_string q.q_class) (q_type_to_string q.q_type)
