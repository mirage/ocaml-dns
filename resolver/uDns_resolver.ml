...
module K = struct
  type t = Dns_enum.rr_typ * Domain_name.t
...
let build_query ?id ?(recursion_desired = false) t ts proto q retry zone edns ip =
  ...
  let transit =
    if QM.mem k t.transit then
      Logs.warn (fun m -> m "overwriting transit of %a (%a)" Domain_name.pp (snd k) Dns_enum.pp_rr_typ (fst k)) ;
...
let maybe_query ?recursion_desired t ts retry out ip typ name (proto, zone, edns, orig_s, orig_p, orig_q, orig_id) =
  ...
  if QM.mem k t.queried then
    ...
    let packet, _ = Dns_packet.encode ?edns proto hdr packet in
    `Query (packet, ip), t
...
let find_queries t k =
  ...
  | exception Not_found ->
    Logs.warn (fun m -> m "couldn't find entry %a (%a) in map"
                  Domain_name.pp (snd k) Dns_enum.pp_rr_typ (fst k)) ;
    s := { !s with drops = succ !s.drops } ;
    t, []
  | vals ->
    QM.remove k t, vals
...
let handle_query t its out ?(retry = 0) proto edns from port ts q qid =
  if Int64.sub ts its > Int64.shift_left retry_interval 2 then begin
    Logs.warn (fun m -> m "dropping q %a from %a:%d (timed out)"
                  Dns_packet.pp_question q Ipaddr.V4.pp from port) ;
    ...
    | `Query (zone, nam, typ, ip) ->
      Logs.debug (fun m -> m "have to query (zone %a) %a (%a) using ip %a"
                     Domain_name.pp zone
                     Domain_name.pp nam Dns_enum.pp_rr_typ typ Ipaddr.V4.pp ip) ;
...
let resolve t ts proto sender sport header v opt =
  ...
  | v ->
    Logs.err (fun m -> m "ignoring %a %a" Dns_packet.pp_header header Dns_packet.pp_v v) ;
    Error (error Dns_enum.FormErr)
          ...
          | Some (zone, edns) ->
            ...
            | `Query_without_edns ->
              ...
              Logs.debug (fun m -> m "resolve: requery without edns %a %a %a"
                             Ipaddr.V4.pp sender Dns_packet.pp_header header
                             Dns_packet.pp_v packet) ;
              let cs, _ = Dns_packet.encode `Udp header packet in
            ...
            | `Upgrade_to_tcp cache ->
              ...
              Logs.debug (fun m -> m "resolve: upgrade to tcp %a %a %a"
                             ...
                             Dns_packet.pp_v packet) ;
              let cs, _ = Dns_packet.encode `Tcp header packet in
        ...
        begin match Dns_resolver_cache.answer t.cache ts q header.Dns_packet.id with
          | `Query (name, cache) ->
            ...
            Logs.debug (fun m -> m "found ip %a, maybe querying for %a (%a)"
                           Ipaddr.V4.pp ip Dns_enum.pp_rr_typ q.Dns_packet.q_type Domain_name.pp name) ;
...
let handle_error ?(error = Dns_enum.FormErr) proto sender sport buf =
  ...
  | Ok header ->
    ...
    in
    match Dns_packet.error header empty error with
    | None -> []
...
let query_root t now proto =
  ...
  in
  let ip, cache =
    match Dns_resolver_cache.find_ns t.cache t.rng now Domain_name.Set.empty q_name with
